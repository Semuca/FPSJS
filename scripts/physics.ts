...
export class PhysicsScene {
    ...
    for (let i = 0; i < this.pobjecs.length; i++) {
      ...
      if (this.CheckForCollision(newPos, i, sizesX, sizesY, sizesZ) == true) {
        ...
        if (Math.abs(position[0] - this.pobjecs[i].position[0]) > sizesX) {
          //If an axis already intersects, ignore checking for intersections.
        ...
        if (x_t > y_t && x_t > z_t) {
          if (Math.abs(newPos[0] - position[0]) > Math.abs(x_t * movement[0])) {
            //If the distance moved so far is greater than the distance you want to move
          ...
          //When the values are all equal?? Right now I'm only dealing with the case (0, 0, 0), as it is by far the most common
          if (Math.abs(position[0] - this.pobjecs[i].position[0]) >= sizesX) {
            if (Math.abs(newPos[0] - position[0]) > Math.abs(x_t * movement[0])) {
              //If the distance moved so far is greater than the distance you want to move
              newPos[0] = position[0] + x_t * movement[0];
  ...
  CheckForCollision(newPos: vec3, secondPobj: number, _sX: number, _sY: number, _sZ: number) {
    // Since both pobecs are cubes of size 1m^3, this will hold
    if (
      Math.abs(newPos[0] - this.pobjecs[secondPobj].position[0]) < _sX && //Checks if we intersect on every axis, if so, we intersect in 3d
...
export class PhysicsObjec {
  // How do we do circles?. How do we check for concavity?
  ...
  Move(direction: vec3): void {
    if (this.enabled === true) {
      const newPos = this.physicsScene.ProposedMove(this.position, direction, this.id);
      vec3.copy(this.position as vec3, newPos);
      ...
      vec3.add(this.position as vec3, this.position as vec3, direction);
    }
  }
}
