...
exports.Shader = exports.Camera = void 0;
...
class Camera {
    ...
    rotpos = new objec_js_1.RotPos([0.0, 0.0, 0.0], undefined, [1.0, 1.0, 1.0]);
    ...
    RecalculateProjMatrix() {
        if (this.type == "2D") {
            ...
            gl_matrix_1.mat4.ortho(this.projectionMatrix, (this.pxWidth * this.zoom) / 2, (-this.pxWidth * this.zoom) / 2, (-this.pxHeight * this.zoom) / 2, (this.pxHeight * this.zoom) / 2, -1.0, 1.0);
        }
        else {
            gl_matrix_1.mat4.perspective(this.projectionMatrix, this.fieldOfView, this.aspectRatio, this.zNear, this.zFar);
        }
        this.SetUniform("uProjectionMatrix", this.projectionMatrix);
    }
    //Change view matrix when camera moves
    UpdatePos() {
        if (this.rotpos.position.length != 3)
            return;
        gl_matrix_1.mat4.fromRotationTranslation(this.viewMatrix, this.rotpos.rotation, this.rotpos.position);
    ...
    CursorToWorldPosition(cursorPosition) {
        ...
        return new geometry_js_1.Point2D(posX, posY);
    }
    SetUniform(uniform, property) {
        ...
        this.window.shaders.forEach((shader) => {
            if (shader.programInfo.uniformLocations[uniform]) {
                ...
                this.window.gl.uniformMatrix4fv(shader.programInfo.uniformLocations[uniform], false, property);
            }
        });
    }
...
class Shader {
    ...
    constructor(screen, gl, camera, type) {
        this.screen = screen;
        this.gl = gl;
        this.camera = camera;
        this.type = type;
    ...
    CreateModel(name, modelData) {
        this.models[name] = new objec_js_1.Model(modelData);
    ...
    InstanceObject(name, rotpos, physicsScene, worldIndex = 0, texName, tags = []) {
        const newObject = new objec_js_1.Objec(rotpos, worldIndex);
    ...
    AdditionalSetup() {
        ...
        this.screen.gl.uniformMatrix4fv(this.programInfo.uniformLocations.uProjectionMatrix, false, this.camera.projectionMatrix);
        if (this.programInfo.uniformLocations.uViewMatrix != undefined) {
            this.gl.uniformMatrix4fv(this.programInfo.uniformLocations.uViewMatrix, false, this.camera.viewMatrix);
        }
    }
    DrawScene(worldIndex) {
        ...
        Object.values(this.models).forEach((model) => {
            ...
            const mode = model.modelData["TEXTURE"] != undefined
                ? this.gl.TRIANGLES
                : this.gl.LINES;
            ...
            model.objects.forEach((object) => {
                ...
                this.gl.uniformMatrix4fv(this.programInfo.uniformLocations.uModelMatrix, false, object.GetMatrix());
                if (model.modelData["ELEMENT_ARRAY_BUFFER"]) {
                    ...
                    this.gl.drawArrays(mode, offset, model.modelData["ARRAY_BUFFER"]["aVertexPosition"][0].length /
                        model.modelData["ARRAY_BUFFER"]["aVertexPosition"][1]); //bad hardcoding, oh well
                }
