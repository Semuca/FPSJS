...
class Sprite extends geometry_js_1.Point2D {
    texture;
...
class Wall extends geometry_js_1.Segment2D {
    texture;
...
function UpdateSidebar(sidebarIndex) {
    ...
    textureGroup.forEach((texture, i) => {
        temp.shaders[0].InstanceObject("verSprite.json", new objec_js_1.RotPos([width * 2 - ((i % 4) + 1) * width + width / 2, sidebar.pxHeight / 2 - width * (Math.floor(i / 4) + 1) + width / 2, 0], Math.PI, [width / 2, width / 2]), physicsScene, 1, texture);
    });
    selector = temp.shaders[0].InstanceObject("verSprite.json", new objec_js_1.RotPos([width * 2 - width + width / 2, sidebar.pxHeight / 2 - width + width / 2, 1.0], Math.PI, [width / 2, width / 2]), physicsScene, 1, "tframe.png");
    DrawSidebar();
}
cam.PreDraw();
Setup();
async function Setup() {
    ...
    const sidepaneData = await (0, loading_js_1.LoadFileText)("../sidepanes.json");
    sidepanes = JSON.parse(sidepaneData).sidepanes;
    await Promise.allSettled([sidepanes.map((sidepane) => sidepane.textures).flat().map((texture) => (0, loading_js_1.CreateTexture)(temp, texture)), (0, loading_js_1.CreateTexture)(temp, "tframe.png")]);
    // Load sidebar
    UpdateSidebar(currentSidepaneIndex);
    highlighter = temp.shaders[0].InstanceObject("verSprite.json", new objec_js_1.RotPos([0.5, 0.5, 1.0], Math.PI, [10, 10]), physicsScene, 0, "tframe.png");
    highlighter.hidden = true;
    secondHighlighter = temp.shaders[0].InstanceObject("verSprite.json", new objec_js_1.RotPos([0.5, 0.5, 1.0], Math.PI, [10, 10]), physicsScene, 0, "tframe.png");
    secondHighlighter.hidden = true;
    hover = temp.shaders[0].InstanceObject("verSprite.json", new objec_js_1.RotPos([0.5, 0.5, 1.0], Math.PI, [20, 20]), physicsScene, 0, "texture.png");
    hover.hidden = true;
    //Load line models
    await (0, loading_js_1.LoadShader)(cam, "2DflatlineVertexShader.vs", "lineFragmentShader.fs");
    modelData = await (0, loading_js_1.LoadModel)(temp, "flatline.json");
    temp.shaders[1].CreateModel("flatline.json", modelData);
    line = temp.shaders[1].InstanceObject("flatline.json", new objec_js_1.RotPos([0.0, 0.0, 0.0], undefined, [0.0, 0.0]), physicsScene, 0);
...
function RenderLoop(now) {
    ...
    gl_matrix_1.quat.setAxisAngle(line.rotpos.rotation, [0.0, 0.0, 1.0], Math.PI);
    ...
    for (let i = 0; i < (cam.pxWidth * cam.zoom - offsetX) / 50.0; i++) {
        temp.gl.uniformMatrix4fv(temp.shaders[1].programInfo.uniformLocations.uModelMatrix, false, line.GetMatrix());
    ...
    gl_matrix_1.quat.setAxisAngle(line.rotpos.rotation, [0.0, 0.0, 1.0], Math.PI / 2);
    ...
    walls.forEach((wall, index) => {
        ...
        gl_matrix_1.quat.setAxisAngle(line.rotpos.rotation, [0.0, 0.0, 1.0], angle);
        temp.gl.uniformMatrix4fv(temp.shaders[1].programInfo.uniformLocations.uModelMatrix, false, line.GetMatrix());
        temp.gl.drawArrays(temp.gl.LINES, 0, 2);
    });
...
cam.onMouseDown = () => {
    if (sidepanes[currentSidepaneIndex].place === "point" && highlighter.hidden === false) {
        sprites.push(new Sprite(new geometry_js_1.Point2D(highlighter.rotpos.position[0] / 50, highlighter.rotpos.position[1] / 50), sidepanes[currentSidepaneIndex].textures[tile]));
        temp.shaders[0].InstanceObject("verSprite.json", new objec_js_1.RotPos([highlighter.rotpos.position[0], highlighter.rotpos.position[1], 0], Math.PI, [25, 25]), physicsScene, 0, sidepanes[currentSidepaneIndex].textures[tile]);
        requestAnimationFrame(RenderLoop);
    }
...
cam.onMouseMove = (e) => {
    ...
    if (mode === MODES.PLACE) {
        ...
        const roundedPoint = new geometry_js_1.Point2D((0, geometry_js_1.roundToNearest)(cursorWorldPosition.x, sidepanes[currentSidepaneIndex].selector), (0, geometry_js_1.roundToNearest)(cursorWorldPosition.y, sidepanes[currentSidepaneIndex].selector));
        if ((0, geometry_js_1.distancePointToPoint)(cursorWorldPosition, roundedPoint) <= highlightRadiusTrigger) {
            if (highlighter.rotpos.position[0] != roundedPoint.x * 50 || highlighter.rotpos.position[1] != roundedPoint.y * 50 || highlighter.hidden === true) {
                ...
                currentPoint = new geometry_js_1.Point2D(roundedPoint.x, roundedPoint.y);
        ...
        highlightedWall = walls.findIndex((wall) => (0, geometry_js_1.ShortestDistanceFromPointToSegment)(cursorWorldPosition, wall) <= highlightRadiusTrigger);
        ...
        // Calculate from position, if a point is enabled or not
        // Logic: figure out the closest point, figure out if that's in range
        const roundedPoint = new geometry_js_1.Point2D(Math.round(cursorWorldPosition.x), Math.round(cursorWorldPosition.y));
        if ((0, geometry_js_1.distancePointToPoint)(cursorWorldPosition, roundedPoint) <= highlightRadiusTrigger) {
            if (secondHighlighter.rotpos.position[0] != roundedPoint.x * 50 || secondHighlighter.rotpos.position[1] != roundedPoint.y * 50 || secondHighlighter.hidden === true) {
                ...
                secondPoint = new geometry_js_1.Point2D(roundedPoint.x, roundedPoint.y);
...
cam.onMouseUp = (e) => {
    if (mode === MODES.DRAWING) {
        if (currentPoint && secondPoint) {
            ...
            secondHighlighter.hidden = true;
            if (secondPoint)
                walls.push(new Wall(currentPoint, secondPoint, sidepanes[currentSidepaneIndex].textures[tile]));
            cam.onMouseMove(e);
        }
    }
    ...
    input.onchange = async (e) => {
        ...
        file.objects.forEach((object) => {
            if (object.tags.includes("wall")) {
                ...
                const angle = gl_matrix_1.quat.getAxisAngle(rotationVec, object.rotation);
                const scale = object.scale[0];
                const xBonus = Math.cos(angle) * scale;
                const yBonus = Math.sin(angle) * scale * (rotationVec[1] < 0 ? 1 : -1);
                const point1 = new geometry_js_1.Point2D(object.position[2] + yBonus, object.position[0] + xBonus);
                const point2 = new geometry_js_1.Point2D(object.position[2] - yBonus, object.position[0] - xBonus);
                walls.push(new Wall(point1, point2, object.texture));
            }
            else if (object.tags.includes("sprite")) {
                sprites.push(new Sprite(new geometry_js_1.Point2D(object.position[0], object.position[2]), object.texture));
                temp.shaders[0].InstanceObject("verSprite.json", new objec_js_1.RotPos([object.position[0] * 50, object.position[2] * 50, 0], Math.PI, [25, 25]), physicsScene, 0, object.texture);
            }
        });
    };
    input.click();
...
temp.keyDownCallbacks["KeyC"] = () => {
    ...
    const text = JSON.stringify({
        ...
        objects: walls.map((wall) => {
            ...
            gl_matrix_1.quat.rotateY(q, q, -Math.atan(wall.gradient));
            return {
                object: "plane.json",
...
temp.keyDownCallbacks["Enter"] = screen_js_1.toggleFullScreen;
