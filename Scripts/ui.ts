...
export function DisplayBox(camera: Camera, target: Shader, scale: [number, number]) {
    const width = scale[0] * camera.pxWidth / 2;
    const height = scale[1] * camera.pxHeight / 2;

    const underPercent = -0.25;

    target.InstanceObject("verSprite.json", new RotPos2D([0.0, underPercent * camera.pxHeight], Math.PI, [width, height]), 0, "white.png");
    target.InstanceObject("verSprite.json", new RotPos2D([0.0, underPercent * camera.pxHeight], Math.PI, [width - 15.0, height - 15.0]), 0, "black.png");
}