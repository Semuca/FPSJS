...
async function Setup() {
    ...
    textureGroup = await (0, loading_js_1.LoadFileText)("../textures.txt");
    textureGroup = textureGroup.split("\n");
    for (let i = 0; i < textureGroup.length; i++) {
        await (0, loading_js_1.CreateTexture)(temp, textureGroup[i] + ".png");
    }
    // Load sidebar
    let width = sidebar.pxWidth / 4;
    for (let i = 0; i < textureGroup.length; i++) {
        temp.shaders[0].InstanceObject("verSprite.json", new objec_js_1.RotPos([sidebar.pxWidth / 2 - ((i % 4) + 1) * width + sidebar.pxWidth / 8, sidebar.pxHeight / 2 - width * (Math.floor(i / 4) + 1) + sidebar.pxWidth / 8, 0.0], Math.PI, [sidebar.pxWidth / 8, sidebar.pxWidth / 8]), physicsScene, 1, textureGroup[i] + ".png");
    }
    sprites = temp.shaders[0].models["verSprite.json"].objects;
    await (0, loading_js_1.CreateTexture)(temp, "tframe.png");
    selector = temp.shaders[0].InstanceObject("verSprite.json", new objec_js_1.RotPos([sidebar.pxWidth / 2 - width + sidebar.pxWidth / 8, sidebar.pxHeight / 2 - width + sidebar.pxWidth / 8, 1.0], Math.PI, [sidebar.pxWidth / 8, sidebar.pxWidth / 8]), physicsScene, 1, "tframe.png");
    //Load line models
    ...
    temp.shaders[1].InstanceObject("flatline.json", new objec_js_1.RotPos([0.0, 0.0, 0.0], undefined, [0.0, 0.0]), physicsScene, 0);
...
document.addEventListener("mousedown", e => {
    ...
    if (e.pageX < cam.pxWidth - 5 && mode === MODES.PLACE) {
        ...
        if (tiles[posX][posY] != undefined) {
            ...
            tiles[posX][posY] = temp.shaders[0].InstanceObject("verSprite.json", new objec_js_1.RotPos([posX * 50.0 + 25.0, posY * 50.0 + 25.0, 0.0], Math.PI, [25.0, 25.0]), physicsScene, 0, textureGroup[tile] + ".png");
        }
        requestAnimationFrame(RenderLoop);
    }
